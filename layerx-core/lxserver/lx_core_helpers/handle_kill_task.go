...
func KillTask(state *lxstate.State, tpiUrl, taskProviderId, taskId string) error {
	if _, err := state.GetTaskFromAnywhere(taskId); err != nil {
		...
		if err != nil {
			return errors.New("sending TASK_KILLED status to task provider"+taskProviderId, err)
		}
		return nil
	}
	...
	if taskPool == state.PendingTaskPool || taskPool == state.StagingTaskPool {
		...
		if err != nil {
			return errors.New("deleting task from staging or pending pool after kill was requested", err)
		}
	...
	if err != nil {
		return errors.New("getting node for task "+taskId, err)
	}

	rpi, err := state.RpiPool.GetRpi(node.GetRpiName())
	if err != nil {
		return errors.New("getting rpi "+node.GetRpiName(), err)
	}
	err = rpi_messenger.SendKillTaskRequest(rpi.Url, taskId)
	if err != nil {
		return errors.New("rpi did not respond to kill task request", err)
	}

	taskToKill.KillRequested = true
	err = taskPool.ModifyTask(taskId, taskToKill)
	if err != nil {
		return errors.New("could not task with KillRequested set back into task pool", err)
	}
	return nil
}
...
func sendTaskKilledStatus(state *lxstate.State, taskState mesosproto.TaskState, tpiUrl, taskProviderId, taskId string) error {
	...
	if err != nil {
		return errors.New("finding task provider for kill request", err)
	}
	...
	if err != nil {
		return errors.New("udpating tpi with TASK_KILLED status for task before task staging was complete", err)
	}
	return nil
}
