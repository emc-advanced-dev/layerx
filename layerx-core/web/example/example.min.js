    ...
    var savedState = `
        <div id='p_tasks' class='containerlike'>
          <div id='pending-tasks' class='container movable'>
          </div>
        </div>
        <div id='s_tasks' class='containerlike'>
          <div id='staging-tasks' class='container non-movable'>
          </div>
        </div>
        <div id='nodes' class='containerlike'>
        </div>
            `
    ...
    xhttp.onreadystatechange = function() {
        if (xhttp.readyState == 4) {
            var innerHtml = `<div class='non-movable title_box title_box_middle'>Pending Tasks</div>`
            ...
            for (i = 0; i < pendingTasks.length; i++) {
                ...
                innerHtml += "<div task_info_id="+task.task_id+" class='task_info clickable movable'><code><B>TASK: </B>"+task.name+"</code><br><code><B>CPUS: </B>"+task.cpus+"</code><br><code><B>MEM: </B>"+task.mem+"</code><br><code><B>DISK: </B>"+task.disk+"</code><br><code><B>FROM: </B>"+task.task_provider.id+"</code><br><code><B>STATUS: </B>TASK_PENDING</code></div>"
            }
    ...
    function refreshStagingTasks() {
        ...
        xhttp.onreadystatechange = function() {
            if (xhttp.readyState == 4) {
                var innerHtml = `<div class='non-movable title_box title_box_middle'>Staging Tasks</div>`
                var pendingTasksObj = JSON.parse(xhttp.responseText)
                ...
                for (i = 0; i < pendingTasks.length; i++) {
                    var task = pendingTasks[i]
                    taskJsonDataMap[task.task_id] = task;
                    innerHtml += "<div task_info_id="+task.task_id+" class='task_info clickable'><code><B>TASK: </B>"+task.name+"<br></code><code><B>CPUS: </B>"+task.cpus+"</code><br><code><B>MEM: </B>"+task.mem+"</code><br><code><B>DISK: </B>"+task.disk+"</code><br><code><B>FROM: </B>"+task.task_provider.id+"</code><br><code><B>STATUS: </B>TASK_STAGING</code></div>"
                }
        ...
        xhttp.onreadystatechange = function() {
            if (xhttp.readyState == 4) {
                ...
                for (i = 0; i < nodes.length; i++) {
                    ...
                    innerHtml += "<div class='container movable'>"
                    innerHtml +=`
                    <div id='`+node.id+`' node_info_id='`+node.id+`' class='container movable node'>
                        <div class='non-movable title_box'>
                            <div class='alignment_left'>Node `+i+`<br>CPUS: `+cpus+`<br>MEM: `+mem+`<br>DISK: `+disk+`<br>TYPE: `+resource_type+`</div>
                        </div>`

                    Object.keys(node.tasks).forEach(function (key) {
                        var task = node.tasks[key]
                        taskJsonDataMap[task.task_id] = task;
                        innerHtml += "<div task_info_id="+task.task_id+" class='task_info clickable movable nodetask'><code><B>TASK: </B>"+task.name+"</code><br><code><B>CPUS: </B>"+task.cpus+"</code><br><code><B>MEM: </B>"+task.mem+"</code><br><code><B>DISK: </B>"+task.disk+"</code><br><code><B>FROM: </B>"+task.task_provider.id+"</code><br><code><B>STATUS: </B>"+taskStatusJsonDataMap[task.task_id]+"</code></div>"
                    })
                    innerHtml += "</div></div>"
                }
    ...
    function migrateTask(nodeId, taskId) {
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
            if (xhttp.readyState == 4) {
                //alert("submitted "+taskId+" to node "+nodeId)
        ...
        xhttp.open("POST", "/MigrateTasks?t=" + Math.random(), true);
        xhttp.send('{"destination_node_id":"'+nodeId+'","task_ids":["'+taskId+'"]}');
    }
    ...
    function clickHandler (e) {
      ...
      target.innerHTML += ' [click!]';
        ...
        var status_value = typeof taskStatusJsonDataMap[task.task_id] != "undefined" ? taskStatusJsonDataMap[task.task_id] : "TASK_PENDING";

        //var innerHtml = taskJsonIntoVerboseHtml(target.getAttribute("task_info"));
        $('task-info').innerHTML = "<div><center><div class='movable title_box title_box_middle'>Task Info</div></center>"
            +"<div class='task_info'><code><B>TASK: </B>"+task.name
            +"</code><br><code><B>CPUS: </B>"+task.cpus
            +"</code><br><code><B>MEM: </B>"+task.mem
            +"</code><br><code><B>DISK: </B>"+task.disk
            +"</code><br><code><B>TASK PROVIDER: </B>"+task.task_provider.source
            +"</code><br><code><B>ENV: </B>"+JSON.stringify(task.command.environment.variables)
            +"</code><br><code><B>COMMAND: </B>"+task.command.value
            +"</code><br><code><B>FROM: </B>"+task.task_provider.id
            +"</code><br><code><B>STATUS: </B>"+status_value
            +"</code></div>"
...
/*
    function taskJsonIntoVerboseHtml(taskJson) {
        ...
        var innerHtml = "foo"
        innerHtml += "<div class='task_info'><code><B>TASK: </B>"+task.name+
            "</code><br><code><B>CPUS: </B>"+task.cpus+
            "</code><br><code><B>MEM: </B>"+task.mem+
            "</code><br><code><B>DISK: </B>"+task.disk+
            "</code><br><code><B>TASK PROVIDER: </B>"+task.task_provider.source+
            ...
            "</code><br><code><B>COMMAND: </B>"+task.command.value+
            "</code><br><code><B>FROM: </B>"+task.task_provider.id+
            "</code><br><code><B>STATUS: </B>TASK_STAGING</code></div>"
        return innerHtml;
    }
*/
    function $ (id) {
    ...
    function addBehaviors() {
        ...
        dragula(containers, {
            ...
            .on('drop', function (el, target, source) {
                ...
                try {
                    console.log("source was "+source.id)
                    var nodeId = target.getAttribute("node_info_id")
                    var taskId = el.getAttribute("task_info_id")
                    if (source == $('pending-tasks')) {
                        assignTask(nodeId, taskId)
                    } else {
                        migrateTask(nodeId, taskId)
                        console.log(source, $('pending-tasks'), "not the same")
                    }
                } catch (e) {
        ...
        addNodeTaskBehaviors()
    }


    function addNodeTaskBehaviors() {
        var containers = []
        var nodeTasks = getEls("nodetask")
        for (var i in nodeTasks) {
            try {
                containers = containers.concat([$(nodeTasks[i].id)])
            } catch (e) {
                console.log("failed to add nodetask to containers", e)
            }
        }
        ...
        for (var i in nodeTasks) {
            if (nodeTasks[i]) {
                crossvent.add(nodeTasks[i], 'click', clickHandler);
            }
        }
    }
    ...
    function addEventEasy (el, type, fn, capturing) {
      try {
          ...
          console.log(e, el, type, fn, capturing)
      }
    }
