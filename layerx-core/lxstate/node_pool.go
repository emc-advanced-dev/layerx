...
func (nodePool *NodePool) Initialize() error {
	...
	if err != nil {
		return errors.New("initializing "+nodePool.GetKey()+" directory", err)
	}
	return nil
}
...
func (nodePool *NodePool) AddNode(node *lxtypes.Node) error {
	...
	if err == nil {
		return errors.New("node "+nodeId+" already exists in database", err)
	}
	err = nodePool.saveNode(node)
	if err != nil {
		return errors.New("saving node to database", err)
	}
	return nil
}
...
func (nodePool *NodePool) GetNode(nodeId string) (*lxtypes.Node, error) {
	...
	if err != nil {
		return nil, errors.New("retrieving node "+nodeId+" from database", err)
	}
	node, err := nodePool.loadNode(nodeId)
	if err != nil {
		return nil, errors.New("loading node "+nodeId+" from stored information in database", err)
	}
	return node, nil
}
...
func (nodePool *NodePool) GetNodes() (map[string]*lxtypes.Node, error) {
	...
	if err != nil {
		return nil, errors.New("retrieving node map from database", err)
	}
...
func (nodePool *NodePool) DeleteNode(nodeId string) error {
	_, err := lxdatabase.GetSubdirectories(nodePool.GetKey() + "/" + nodeId)
	if err != nil {
		return errors.New("retrieving node "+nodeId+" from database", err)
	}
	err = lxdatabase.Rmdir(nodePool.GetKey()+"/"+nodeId, true)
	if err != nil {
		return errors.New("recursivey removing directory "+nodePool.GetKey()+"/"+nodeId+" from database", err)
	}
	return nil
}
...
func (nodePool *NodePool) GetNodeResourcePool(nodeId string) (*ResourcePool, error) {
	if _, err := nodePool.GetNode(nodeId); err != nil {
		return nil, errors.New("could not find node", err)
	}
	return &ResourcePool{
		nodeId:  nodeId,
...
func (nodePool *NodePool) saveNode(node *lxtypes.Node) error {
	...
	if err != nil {
		return errors.New("initializing "+nodePool.GetKey()+"/"+nodeId+" directory", err)
	}
	...
	for _, resource := range node.Resources {
		...
		if err != nil {
			return errors.New("adding resource "+resource.Id+" to node "+nodeId+" resource pool", err)
		}
	}
	...
	for _, task := range node.RunningTasks {
		...
		if err != nil {
			return errors.New("adding task "+task.TaskId+" to node "+nodeId+" task pool", err)
		}
	}
	return nil
}
...
func (nodePool *NodePool) loadNode(nodeId string) (*lxtypes.Node, error) {
	...
	if err != nil {
		return nil, errors.New("could not get list of resources for node "+nodeId, err)
	}
	for _, resource := range resources {
		err = node.AddResource(resource)
		if err != nil {
			return nil, errors.New("could not add resource to node object", err)
		}
	}
	nodeTaskPool := TaskPool{
		rootKey: nodePool.GetKey() + "/" + nodeId + "/running_tasks",
	...
	if err != nil {
		return nil, errors.New("could not get list of tasks for node "+nodeId, err)
	}
	for _, task := range tasks {
		err = node.AddTask(task)
		if err != nil {
			return nil, errors.New("could not add task to node object", err)
		}
	}
	return node, nil
}
