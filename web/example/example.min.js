...
var taskStatusJsonDataMap = new Array();
...
var savedState = `
<div id='state' class='wrapper'>
    <div id='pending-tasks' class='container movable'>
    <div class='non-movable'><h1>Pending Tasks</h1></div>

</div>
<div id='staging-tasks' class='container'>
    <div class='non-movable'><h1>Staging Tasks</h1></div>
</div>
<div id='nodes' class='container'>
    <div class='non-movable'><h1>Nodes</h1></div>
</div>
</div>`
...
for (var i in coll) {
    //TODO
    //if ((containers[i]) && (!containers[i].id.includes("undefined"))){
    //    containers = containers.concat([$(coll[i].id)])
    //    alert("1st time doing containers "+containers[i]+" "+containers[i].id+" "+containers[i].innerHTML)
    //    crossvent.add(containers[i], 'click', clickHandler);
    //    alert("finished doing container "+containers[i]+" "+containers[i].id+" "+containers[i].innerHTML)
    //}
    containers = containers.concat([$(coll[i].id)])
    try{
        alert("adding click handler for "+containers[i])
        crossvent.add(containers[i], 'click', clickHandler);
        alert("added"+" ("+containers[i].id+")")
    }catch(e){
        console.log("YO",e, containers[i])
    }
}
...
function refreshTaskStatuses() {
    ...
    xhttp.onreadystatechange = function() {
        if (xhttp.readyState == 4) {
            var taskStatusesObj = JSON.parse(xhttp.responseText)
            var taskStatuses = taskStatusesObj.sort(function(a, b) {
            ...
            for (i = 0; i < taskStatuses.length; i++) {
                ...
                taskStatusJsonDataMap[taskStatus.task_id] = taskStatusName;
            }
        }
    };
...
xhttp.onreadystatechange = function() {
    if (xhttp.readyState == 4) {
        ...
        for (i = 0; i < pendingTasks.length; i++) {
            ...
            innerHtml += "<div task_info_id="+task.task_id+">Task "+task.name+"<br><code>CPUS:"+task.Cpus+"</code><br><code>MEM:"+task.Mem+"</code><br><code>DISK:"+task.Disk+"</code><br><code>STATUS: TASK_PENDING</code></div>"
        }
    ...
    xhttp.onreadystatechange = function() {
        if (xhttp.readyState == 4) {
            ...
            for (i = 0; i < pendingTasks.length; i++) {
                var task = pendingTasks[i]
                taskJsonDataMap[task.task_id] = task;
                innerHtml += "<div task_info_id="+task.task_id+">Task "+task.name+"<br><code>CPUS:"+task.Cpus+"</code><br><code>MEM:"+task.Mem+"</code><br><code>DISK:"+task.Disk+"</code><br><code>STATUS:"+taskStatusJsonDataMap[task.task_id]+"/code></div>"
            }
...
function refreshNodes() {
    ...
    xhttp.onreadystatechange = function() {
        if (xhttp.readyState == 4) {
            var innerHtml = `<div class='non-movable'><h1>Nodes</h1></div>`;
            ...
            for (i = 0; i < nodes.length; i++) {
                ...
                innerHtml +=  `
 <div id='`+`node.id`+`' class='container movable'>
     <div class='non-movable'><h1>Node `+i+`<br><code>CPUS:`+cpus+`</code><br><code>MEM:`+mem+`</code><br><code>DISK:`+disk+`</code></h1></div>`

              //  for (j = 0; j < nodes.tasks.length; j++) {
              //      var task = nodes.tasks[i]
              //      innerHtml += "<div task_info_id="+task.task_id+">Task "+task.name+"<br><code>CPUS:"+task.Cpus+"</code><br><code>MEM:"+task.Mem+"</code><br><code>DISK:"+task.Disk+"</code><br><code>STATUS:"+taskStatusJsonDataMap[task.task_id]+"/code></div>"
              //  }
              innerHtml += `</div>`
            }
...
function clickHandler (e) {
  ...
  target.innerHTML += ' [click!]';
    ...
    $('task-info').innerHTML = "<div><div class='non-movable'><h1>Task Info</h1></div>"
        +"<div>Task "+task.name
